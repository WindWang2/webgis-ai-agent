  ...
  return (
    <div className="my-2 p-3.5 rounded-xl border border-slate-200/80 bg-white/80 dark:border-slate-800 dark:bg-slate-900/80 backdrop-blur-md text-sm shadow-sm transition-all">
      ...
      <div className="flex items-center justify-between mb-2.5">
        ...
        <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-950/60 dark:text-emerald-300 font-mono font-medium">
          R={eps1}m / T={eps2}s
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
        <div data-testid="st-badge-clusters" className="p-2 rounded-lg border bg-emerald-500/10 border-emerald-500/30 flex flex-col justify-between">
          <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">聚类簇数</span>
          <span className="text-sm font-mono font-bold text-emerald-700 dark:text-emerald-300">{totalClusters}</span>
        </div>
        <div data-testid="st-badge-clustered" className="p-2 rounded-lg border bg-blue-500/10 border-blue-500/30 flex flex-col justify-between">
          <span className="text-xs font-bold text-blue-600 dark:text-blue-400">聚类点数</span>
          <span className="text-sm font-mono font-bold text-blue-700 dark:text-blue-300">{clusteredPoints}</span>
        </div>
        <div data-testid="st-badge-noise" className="p-2 rounded-lg border bg-amber-500/10 border-amber-500/30 flex flex-col justify-between">
          <span className="text-xs font-bold text-amber-600 dark:text-amber-400">噪声点数</span>
          <span className="text-sm font-mono font-bold text-amber-700 dark:text-amber-300">{noisePoints}</span>
        </div>
        <div data-testid="st-badge-span" className="p-2 rounded-lg border bg-purple-500/10 border-purple-500/30 flex flex-col justify-between">
          <span className="text-xs font-bold text-purple-600 dark:text-purple-400">跨度(h)</span>
          <span className="text-sm font-mono font-bold text-purple-700 dark:text-purple-300">{temporalSpan}h</span>
        </div>
      </div>

      {/* Timeline Controls */}
      <div className="p-2.5 mb-2.5 rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/60">
        <div className="flex items-center justify-between mb-1.5 text-xs text-slate-500 dark:text-slate-400 font-mono">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            aria-label="重置时间轴"
            onClick={handleReset}
            className="p-1.5 rounded-md bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
          >
