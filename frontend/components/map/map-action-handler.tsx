...
export const MapActionHandler = React.memo(function MapActionHandler() {
  ...
  // Style gate note: the ref is taken INSIDE the whenStyleReady callback — a
  // pass cancelled before the style was ready never claimed ownership, so the
  // re-scheduled pass can proceed instead of stranding the action at the head.
  ...
  // Style gate: useMap() hands out the instance at mount, BEFORE the async
  // style load finishes — mounting the annotation stack in that window throws
  // "Style is not done loading." Defer until ready; the returned cancel is the
  // effect cleanup so a remap/unmount cannot fire a stale mount.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Style gate: the queue head waits for the base style before running
    // (same contract as the refresh effect above). The settle-ownership ref
    // and the run body move inside the callback; effect cleanup cancels a
    // pending wait, and startedAt is captured at actual run start so the
    // style-load wait never inflates the reported command duration.
    return whenStyleReady(map, () => {
      // The wait was cancelled-superseded if an earlier pass already started
      // this action (double protection alongside the cancel path).
      if (runningActionIdRef.current === action.action_id) return;
