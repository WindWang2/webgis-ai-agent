...
function AttributionRenderer(component: MapSpecComponent, _ctx: RendererContext) {
  ...
  if (!text) return null;
  return (
    <div data-testid="spec-chrome-attribution" style={positionStyle(component)} className={`map-chrome absolute z-30 px-2 py-0.5 text-micro text-map-chrome-ink-muted ${positionClass(component)}`}>
      {text}
    </div>
  );
}
