...
/**
 * Exports a MapSpec to a high-definition vector PDF file.
 */
export async function exportMapSpecToVectorPdf(
  ...
  pdf.text(options.title || 'WebGIS AI Agent Vector Map', pageW / 2, 14, { align: 'center' });

  if (options.subtitle) {
    pdf.setFontSize(10);
    pdf.setTextColor(100, 116, 139);
    pdf.text(options.subtitle, pageW / 2, 20, { align: 'center' });
  }

  // Draw border & vector indicator
  pdf.setDrawColor(30, 58, 138);
  pdf.setLineWidth(0.5);
  pdf.rect(10, 24, pageW - 20, pageH - 34);

  pdf.setFontSize(8);
  pdf.setTextColor(148, 163, 184);
  pdf.text('Generated by WebGIS AI Agent (Vector 300+ DPI)', pageW / 2, pageH - 4, { align: 'center' });
