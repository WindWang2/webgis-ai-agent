...
### 1. 工作区上下文面板（V3 IA）
NavRail + ContextPanel 外壳，标签含：**聊天 / 项目 / 数据源 / 图层 / 分析 / 制图工坊 / 任务中心**。
- **会话管理** - 历史抽屉支持两步确认删除会话；新建会话在工作区非空时需确认
- **深度探索** - `explorer_progress` 实时进度面板（任务可关闭、上限 50、随会话切换清理）
- **项目上下文** - 激活项目随聊天请求注入（`project_id`）
- **匿名治理** - 项目写操作对匿名用户禁用并提示登录（后端鉴权不变）
...
- 🎚️ 2D/3D 切换
- 📊 HUD 显示切换
- 💾 导出（DPI 重采样有 30s idle 超时兜底，WebGL context lost 时显式报错并恢复 pixelRatio）

### 3. 底图与瓦片凭据
- 底图卡片直绑 `TILE_PROVIDERS` 目录（OpenTopoMap `{s}` 占位符已修为具体子域）
- MapLibre `transformRequest` 对 first-party 瓦片/图片请求注入会话凭据（MVT >5000 要素图层可加载）
- 导出/报告下载与聊天内嵌图片走认证 blob 传输层（不再裸 `<a>`/`<img>` 401）
