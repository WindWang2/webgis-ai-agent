...
- **D4 版本化 + 旧图兼容。** StandardsPack frozen + semver + fingerprint；显式 profile → 义务 error 级（opt-in 严格）；推断 profile → 同义务降 warning（legacy_map 证据）；`enabled=False` → status=disabled 空报告（feature-off 回归零变化）。证据缺失恒 not_evaluated，不伪造。
