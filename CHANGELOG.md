...
- Service wrapper `app/services/cartography/standards_qa.py`
  ...
  generator).
## [Unreleased] - 2026-09-17 (multiscale-scene-intelligence/v1: scene planning + v1.4 contract + terrain tiles + evidence gates, ADR-0199)

### Added
- Scene planning (`app/lib/cartography/scene_planning.py`): deterministic
  SceneIntent → SceneDecision projection (2d/2.5d/3d). Fail-closed: no height
  attribute evidence → no extrusion; no elevation evidence → no terrain;
  static media (print/PDF/SVG) → no perspective 3D (hillshade 2.5d allowed);
  exaggeration >1.5 disclosed as scale-distorting (hard cap 10).
- MapSpec v1.4 (additive): top-level `scene` (MapSceneConfig/TerrainSceneSpec/
  SceneCameraSpec) + typed `MapSpecLayer.extrusion` (absorbs the converter's
  open dict, adds `elevation_ref` provenance); identity upgrader; TS
  projection regenerated byte-idempotently; legacy specs stay byte-stable.
- `SetSceneIntent` (presentation-surface transaction): COW top-branch write of
  `scene` only — layers/sources/legend_spec/thresholds structurally unchanged
  so 2D↔3D switches cannot drift statistics/classification/legend; strict
  shape + exaggeration-bounds validation with last-known-good on reject;
  facade `mapspec_store.set_scene`.
- Session DEM → MapLibre terrain data plane: terrarium codec
  (`terrain_encoding.py`, known-answer + round-trip tests) and
  `render_terrarium_tile` + `GET /layers/data/{ref}/terrain-tiles/…`.
  Fail-closed: multi-band (`TERRAIN_REQUIRES_SINGLE_BAND`) and CRS-less
  (`TERRAIN_REQUIRES_CRS`) refs rejected; dual-channel reprojection
  (elevation + validity weights, threshold 254.5) so nodata-adjacent
  interpolation cannot fabricate transition elevations; nodata/sentinel →
  transparent, never 0-elevation; errors never cached (422 {code,message}).
- Deterministic scene quality gate (`scene_quality.py`): 9-code closed
  vocabulary (terrain source ref/type blocking, exaggeration range blocking,
  pitch hard cap blocking; evidence-less extrusion / empty-3d-mode / unknown
  vertical unit warnings; distortion disclosure) + `check_legend_invariance`
  per-layer legend digest oracle. VLM 5-axis contract (ADR-0185) untouched.
- Scene self-heal mapping (`scene_selfheal.py`): blocking findings → existing
  mutation intents only (SetViewIntent pitch clamp / SetSceneIntent honest-1.0
  or terrain withdrawal); intent whitelist test-locked; warnings disclose-only.
- Scene degradation chain (`scene_degradation.py`): 3d→2.5d→2d deterministic
  triggers with per-hop info-loss disclosure; COW; 2d terminal.
- Camera planner (`scene_camera.py`): overview/detail/compare roles,
  antimeridian short-arc centers, zoom 3–18, product pitch ≤60, compare
  same-zoom opposing bearings, reduced-motion → zero transitions.
- Scale-aware LOD (`scene_lod.py`): monotonic zoom-band table projecting onto
  existing ADR-0154 label.zoomBands / thresholds.maxFeatures (no new spec
  fields).
- Agent tools (`app/tools/scene_tools.py`): `plan_map_scene` (read-only
  deterministic planner with evidence-honesty boundary) and `set_map_scene`
  (transactional facade write).
- Scene perf benchmarks (`tests/benchmarks/test_scene_perf.py`, marker=perf):
  O(1) planning, 1k-layer gate, 1k² terrarium encode — synthetic data only.
- Degradation vocabulary: `scene_extrusion_no_height_evidence` (info) and
  `scene_terrain_unavailable` (warning) in the authoritative
  `render_diagnostics.py` registry with frontend emitters.

### Changed
- Frontend 3D auto-extrusion is evidence-gated (ADR-0199): is3D auto-extrudes
  a polygon layer only with an explicit `layer.extrusion` contract or a
  numeric feature `height` field; otherwise flat render + bounded
  scene-evidence ring disclosure. The previous `coalesce(get height, 20)`
  fabricated a 20 m height for layers without height data — removed (Oracle:
  no fabricated elevations without evidence).
- map-panel 3D toggle is spec-driven: committed `scene.terrain`
  (exaggeration + raster-dem source) and `scene.camera` (pitch/bearing/
  transition) are honored; without a scene the legacy AWS-terrarium default
  remains (MapLibre fallback always available).
- Headless compiler: `scene.terrain` → MapLibre style `terrain` projection
  (dangling/non-raster-dem source → compile errors, fail-closed); symbol
  layers default to viewport pitch/rotation alignment in 3d scenes (explicit
  declarations never overridden); symbol layout passthrough gap closed
  (icon-*/text-* layout keys were silently dropped).
- `create_3d_extrusion_map` accepts optional `elevation_ref` (height-field
  provenance); converter passes it into `MapSpecLayer.extrusion`.
