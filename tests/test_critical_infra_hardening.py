...
def test_i4_configmap_no_plaintext_secret():
    ...
    只看 yaml 实际的资源类型。
    """
    configmap = (REPO_ROOT / "deploy" / "k8s" / "01-configmap.yaml").read_text(encoding="utf-8")
    # 不应再有 kind: Secret 资源
...
def test_i7_dockerfile_prod_no_copy_all():
    ...
    dockerfile_prod = (REPO_ROOT / "Dockerfile.prod").read_text(encoding="utf-8")
...
def test_i11_prometheus_yml_uses_correct_path():
    ...
    prom_yml = (REPO_ROOT / "deploy" / "prometheus.yml").read_text(encoding="utf-8")
