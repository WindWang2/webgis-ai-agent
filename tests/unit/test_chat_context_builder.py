...
# ─── format_layer_lines (纯，无 session 依赖) ───────────────────────


class TestFormatLayerLines:
    def test_empty_returns_empty(self):
        assert format_layer_lines({}, []) == []

    def test_inventory_priority(self):
        out = format_layer_lines(
    ...
    def test_fallback_to_active_when_no_inventory(self):
        out = format_layer_lines(
            inventory={},
            active_layers=[
...
class TestMapStateSummary:
    def test_empty_session_includes_defaults(self, clean_session):
    ...
    def test_viewport_renders(self, clean_session):
        session_data_manager.set_map_state(clean_session, "viewport", {
    ...
    def test_bounds_render(self, clean_session):
        session_data_manager.set_map_state(clean_session, "viewport", {"bounds": [1.1, 2.2, 3.3, 4.4]})
        out = build_map_state_summary(clean_session)
        assert "可视范围" in out
        assert "1.100" in out and "4.400" in out

    def test_inventory_layers(self, clean_session):
        session_data_manager.store(clean_session, {"type": "FeatureCollection", "features": []}, prefix="data")
        out = build_map_state_summary(clean_session)
        # store 会创建一个 ref，summary 里应当包含
        assert "活跃图层:" in out
        assert "ref:data-" in out

    def test_user_location_renders(self, clean_session):
        session_data_manager.set_map_state(clean_session, "user_location", {"lng": 116.5, "lat": 39.8, "accuracy": 10})
        out = build_map_state_summary(clean_session)
...
class TestComposeRequestMessages:
    def test_injects_env_into_system_prompt(self, clean_session):
        msgs = [
            {"role": "system", "content": "BASE_PROMPT"},
            {"role": "user", "content": "hi"},
        ]
        out = compose_request_messages(clean_session, msgs)
        # 系统提示被合并扩展
    ...
    def test_appends_last_ctx_when_history_nonempty(self, clean_session):
        msgs = [
    ...
    def test_empty_messages_returns_empty(self, clean_session):
        assert compose_request_messages(clean_session, []) == []
