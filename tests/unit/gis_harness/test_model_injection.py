...
class TestNewRecipeSelectionAndEligibility:
    def test_registry_now_has_ten_recipes(self):
        ...
        # Workflow V2（Goal C / ADR-0101）：17 个 V1 seed 之外，领域包
        # 追加专业工作流 recipe（catalog 由 registry 生成，见
        # docs/workflows/workflow-catalog.md）。
        # 2026-09 扩容：+interpolation_uncertainty_map、+rs_scene_inventory
        # → 166（17 + 149）。#1405 锚点与 registry 对齐。
        assert "interpolation_uncertainty_map" in reg.all_ids
        assert "rs_scene_inventory" in reg.all_ids
        assert reg.count == 17 + 149
