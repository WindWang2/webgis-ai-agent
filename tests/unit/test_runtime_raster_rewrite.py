...
def test_assemble_raster_assets_copies_png(tmp_path: Path):
    ...
    copied = assemble_raster_assets(session_dir, dist, mapspec)
    assert copied == 1
...
def test_assemble_raster_assets_missing_dir_returns_zero(tmp_path: Path):
    from app.services.runtime_asset_assembly import assemble_raster_assets

    session_dir = tmp_path / "nope"
    dist = tmp_path / "dist2"
    dist.mkdir()
    assert assemble_raster_assets(session_dir, dist, _sample_mapspec_with_raster()) == 0


def test_assemble_raster_assets_missing_png_skipped(tmp_path: Path):
    from app.services.runtime_asset_assembly import assemble_raster_assets
...
def test_e2e_rewrite_and_copy_produces_origin_url_and_dist_file(tmp_path: Path):
    ...
    """
    from app.services.runtime_asset_assembly import assemble_raster_assets, rewrite_mapspec_raster_refs
    ...
    copied = assemble_raster_assets(session_dir, dist, rewritten)
    assert copied == 1
    assert (dist / "raster" / "ndvi_src.png").read_bytes() == png_bytes
