...
| 阶段 | 内容 | 规模 | 依赖 | 验收标准 |
|---|---|---|---|---|
| **Phase 1** | 三条高杠杆规则(`load.ratio`/`color.separability`/`legend.completeness`)+ typed fix 映射 + 修复动作 | M | 无 | ① 规则单测(golden fixture:超载图/低色差图/缺图例图各自 fail 且 fix 类型正确);② 修复循环端到端:注入失败原因后下一候选通过;③ 现有 L4 评审无回归 |
