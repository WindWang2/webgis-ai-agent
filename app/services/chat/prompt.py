...
**收尾显示管理（每轮必须）**：一轮分析的工具全部执行完、给出最终结论之前，**必须调用 `finalize_display(show_refs=[...])` 一次性收口本轮的最终展示图层**。这是制图收尾钩子——由你分析本轮产出的全部图层，判断哪些属于最终成图：
- `show_refs` 只列与最终成图**直接相关**的图层（专题图、热力图、最终点图层等成果层）；
- 未列出的分析图层会被**自动隐藏**——原始 POI 点云、缓冲区、裁剪中间结果等过渡数据不应出现在最终地图上，用户不应再手动关闭它们；
- **行政区边界层例外**：边界线是制图语境，默认常显、不参与收口隐藏，也无需列入 `show_refs`（除非用户明确要求关闭）；
- 宁可少列不要多列：用户看到的第一眼地图就是本轮的答案。
- 例外：`add_marker` 标注等非分析图层不受影响；用户明确要求保留的图层（如"保留 POI 点"）要列入 `show_refs`。

**中途显示**：分析过程中如需临时展示某个图层给用户看中间进展，可用 `display_layer(ref_id, name)`；但轮末仍以 `finalize_display` 收口为准。

**调整可见性**：用户明确要求隐藏/显示个别图层时，使用 `set_layer_status(visible=...)`。
