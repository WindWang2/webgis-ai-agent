"""Runtime fixture asset assembly — MVT tiles + raster PNGs.
...
``dist/raster/``. The compiler stays session-agnostic. The copy helper
(``copy_session_raster_assets``) mirrors the MVT assembly pattern.
"""
...
def copy_session_raster_assets(session_dir: Path, dist_dir: Path, mapspec: Dict[str, Any] | None = None) -> int:
