...
ONTOLOGY_TASKS: Tuple[TaskDescriptor, ...] = (
    ...
    # ── Epic 11（methodology-template-v2）纯加法补齐：邻近/聚类/时空/
    #    叠加/图集。只引用既有 capability / artifact / map model / intent
    #    词表（校验对账），不改任何既有条目。────────────────────────────
    _t(
        task_id="network.proximity_buffer", domain="network",
        label_zh="邻近与缓冲", label_en="Proximity & buffer",
        description="围绕主体要素的欧氏邻近域（缓冲/多环缓冲）表达与统计；"
                    "路网可达语义请用 network.service_area（欧氏缓冲≠路网可达）。",
        required_data_roles=("subject",),
        optional_data_roles=("network", "boundary"),
        geometry_expectations=("point", "line", "polygon"),
        common_capabilities=("geometry_buffer", "multi_ring_buffer",
                             "proximity_buffer"),
        output_artifacts=("proximity_zone", "polygon_feature_set"),
        cartographic_expectations=("proximity_overlay", "multi_ring_buffer_map"),
        component_expectations=("legend", "title", "scale_bar"),
        ambiguity_rules=(
            AmbiguityRule(
                signal="「范围内/周边」未区分欧氏缓冲与路网可达",
                default_resolution="欧氏缓冲（成本低、数据要求少）并披露局限",
                alternatives=("network.service_area",),
                disambiguator="路网数据在场 + 表述含 车程/步行/分钟",
            ),
        ),
        fallback_strategy=(
            FallbackTier(tier="preferred", condition="主体几何在场且半径明确",
                         cartography=("multi_ring_buffer_map",)),
            FallbackTier(tier="degraded", condition="主体几何存在但投影为地理坐标系",
                         cartography=("proximity_overlay",),
                         disclosure="地理坐标系：缓冲计算内部投影到局部度量系，"
                                    "结果以米制表达。"),
            FallbackTier(tier="minimal", condition="仅属性无几何",
                         disclosure="无几何：仅输出主体的描述统计。"),
            FallbackTier(tier="blocked", condition="无主体数据",
                         blocks_completion=True),
        ),
        family_triggers=("proximity_analysis",),
        analysis_intent_triggers=("proximity_buffer",),
        cartography_triggers=("proximity_overlay",),
        keywords_zh=("缓冲区", "缓冲", "周边", "半径", "覆盖范围", "范围内"),
        keywords_en=("buffer", "within", "radius", "surrounding", "walkshed"),
    ),
    _t(
        task_id="spatial_statistics.point_clustering", domain="spatial_statistics",
        label_zh="点聚类", label_en="Point clustering",
        description="点要素的密度聚类分簇（DBSCAN/NNI）：把点分成簇与噪声；"
                    "与 LISA 局部自相关正交（后者需要 measure 字段）。",
        required_data_roles=("subject",),
        optional_data_roles=("boundary",),
        geometry_expectations=("point",),
        common_capabilities=("point_pattern_analysis",),
        output_artifacts=("hotspot_result", "stats_table"),
        cartographic_expectations=("dbscan_cluster_map", "point_cluster"),
        component_expectations=("legend", "title", "chart_panel"),
        fallback_strategy=(
            FallbackTier(tier="preferred", condition="点样本充足",
                         cartography=("dbscan_cluster_map",)),
            FallbackTier(tier="degraded", condition="样本量偏小",
                         disclosure="样本量偏小：聚类结构仅作探索性参考。"),
            FallbackTier(tier="minimal", condition="无点几何",
                         disclosure="非点几何：不能执行点聚类。"),
            FallbackTier(tier="blocked", condition="无主体数据",
                         blocks_completion=True),
        ),
        family_triggers=("concentration_analysis",),
        analysis_intent_triggers=(),
        cartography_triggers=(),
        keywords_zh=("聚类", "分簇", "聚成几类", "簇", "密度聚"),
        keywords_en=("cluster", "dbscan", "grouping points"),
    ),
    _t(
        task_id="spatial_statistics.spatiotemporal_pattern", domain="spatial_statistics",
        label_zh="时空格局", label_en="Spatiotemporal pattern",
        description="时空交互/时空 K 函数/时空聚类：事件在空间与时间上的"
                    "联合聚集结构（需要时间字段）。",
        required_data_roles=("subject",),
        optional_data_roles=("boundary",),
        geometry_expectations=("point",),
        common_capabilities=("space_time_interaction", "space_time_k_function",
                             "spatiotemporal_clustering"),
        output_artifacts=("stats_table", "hotspot_result"),
        cartographic_expectations=("hotspot_overlay", "temporal_comparison_map"),
        component_expectations=("legend", "title", "chart_panel"),
        fallback_strategy=(
            FallbackTier(tier="preferred", condition="时间字段在场且样本充足",
                         capabilities=("space_time_k_function",)),
            FallbackTier(tier="degraded", condition="时间字段缺失",
                         disclosure="缺少时间维度：退化为纯空间聚集分析。"),
            FallbackTier(tier="minimal", condition="仅空间分布"),
            FallbackTier(tier="blocked", condition="无主体数据",
                         blocks_completion=True),
        ),
        family_triggers=("temporal_trend",),
        analysis_intent_triggers=(),
        cartography_triggers=(),
        keywords_zh=("时空", "时空聚集", "时空格局", "时空分布"),
        keywords_en=("spatiotemporal", "space-time", "space time pattern"),
    ),
    _t(
        task_id="decision.overlay_composite", domain="decision",
        label_zh="叠加合成", label_en="Overlay composite",
        description="多图层几何叠加/相交/裁剪合成（约束面、影响面交集）；"
                    "因子加权请用 multi_criteria（MCDA），不混叠。",
        required_data_roles=("subject", "constraint"),
        optional_data_roles=("boundary",),
        geometry_expectations=("polygon", "line", "point"),
        common_capabilities=("geometry_overlay",),
        output_artifacts=("polygon_feature_set", "raster_surface"),
        cartographic_expectations=("suitability_constraint_overlay",
                                   "classified_raster"),
        component_expectations=("legend", "title"),
        fallback_strategy=(
            FallbackTier(tier="preferred", condition="多图层几何齐备",
                         cartography=("suitability_constraint_overlay",)),
            FallbackTier(tier="degraded", condition="部分图层缺失",
                         disclosure="部分图层缺失：仅对可用图层做叠加并披露。"),
            FallbackTier(tier="minimal", condition="单图层"),
            FallbackTier(tier="blocked", condition="无几何数据",
                         blocks_completion=True),
        ),
        family_triggers=("proximity_analysis",),
        analysis_intent_triggers=("overlay_weighted",),
        cartography_triggers=(),
        keywords_zh=("叠加", "相交", "裁剪", "交集", "叠加分析"),
        keywords_en=("overlay", "intersect", "clip"),
    ),
    _t(
        task_id="cartographic.atlas_reporting", domain="cartographic",
        label_zh="图集与报告", label_en="Atlas & reporting",
        description="多幅专题图 + 统计页组成的产品化图集/报告集"
                    "（版面化、逐幅成图）。",
        required_data_roles=("subject",),
        optional_data_roles=("boundary", "measure"),
        geometry_expectations=("point", "polygon", "line", "raster"),
        common_capabilities=(),
        output_artifacts=("feature_collection", "stats_table", "chart_spec"),
        cartographic_expectations=(),
        component_expectations=("title", "legend", "north_arrow", "scale_bar",
                                "attribution", "chart_panel"),
        fallback_strategy=(
            FallbackTier(tier="preferred", condition="多主题数据齐备",
                         cartography=()),
            FallbackTier(tier="degraded", condition="部分主题数据缺失",
                         disclosure="部分主题缺数据：图集按可用主题组装并披露。"),
            FallbackTier(tier="minimal", condition="单主题"),
            FallbackTier(tier="blocked", condition="无成图数据",
                         blocks_completion=True),
        ),
        family_triggers=("distribution_overview",),
        analysis_intent_triggers=(),
        cartography_triggers=(),
        keywords_zh=("图集", "专题图集", "多张图", "图册", "报告集"),
        keywords_en=("atlas", "map book", "map series"),
    ),
)
