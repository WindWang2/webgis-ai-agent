...
_STR_MAX = 64
#: dependency_available / credentials_present 进入资格上下文的有界上限
#:（与 QualificationContext 消费面一致，超额截断）。
_MAX_DEP_KEYS = 8
_MAX_CRED_KEYS = 8

#: 可用性探针 TTL（秒）—— dispatch 热路径零重复探针；单飞防惊群。
...
def _cached_availability(key: str, fn: Callable[[], Optional[bool]]) -> Optional[bool]:
    """TTL + 单飞缓存的可用性探针（None = unknown，不入缓存）。"""
    now = time.monotonic()
    with _WORKER_PROBE_LOCK:
...
class RuntimeSituation:
    ...
    #: 依赖可用性（键 ≤8）：``celery_broker`` / ``durable_worker`` …
    ...
    def to_qualification_dict(self) -> Dict[str, Any]:
        ...
        if self.dependency_available:
            out["dependency_available"] = dict(
                list(self.dependency_available.items())[:_MAX_DEP_KEYS])
    ...
    def to_bounded_view(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dependency_keys": sorted(self.dependency_available.keys())[:_MAX_DEP_KEYS],
...
def build_runtime_situation(
    ...
    *,
    ...
    try:
        ...
        cred_map, cred_meta = _collect_security_facts()
...
def merge_situation_facts(base: Any, session_id: str) -> Any:
    ...
    if isinstance(base, dict):
        merged = dict(base)
        for key in ("credentials_present", "dependency_available"):
    ...
    try:
        ...
        if isinstance(base, QualificationContext):
            ...
            if not patched.dependency_available and runtime_facts.get(
                    "dependency_available"):
                patched.dependency_available = dict(
                    runtime_facts["dependency_available"])
