"""PlanRuntime —— 计划版本化 / 重规划驱动 / 失败种子最小重算（V7 ADR-0134 D2）。
...
async def request_replan(
    ...
    *,
    ...
    """重规划生产驱动点（finalizer 出口调用；ADR-0134 D2）。
    ...
    # ADR-0213 D5（R6）：replan 同时消耗 governor RetryBudget 令牌 ——
    ...
    try:
        ...
        return payload
    # ADR-0213 D5（R6）：次数闸 + 令牌闸都通过、置位成功后才实扣令牌
