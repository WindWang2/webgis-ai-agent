...
class ChatEngine:
    ...
    def _get_map_state_summary(self, session_id: str) -> str:
        ...
        if inventory:
            ...
            for ref_id, alias in inventory.items():
                ...
                summary += f"  * {ref_id}{alias_str}\n"
        else:
            summary += "- 目前地图上没有活动的图层。\n"
        
        summary += "\n提示：如果用户要求修改图层（透明度、颜色等），请直接引用上述 ID 或别名。"
        return summary
    ...
    async def chat(self, message: str, session_id: Optional[str] = None) -> dict:
        ...
        messages_with_context = messages + [{"role": "system", "content": map_context}]
...
**规则**：
1. **统计即图表**：当你的分析产生数值结果时，**必须主动**展示相关图表或专题地图。
...
## 重要规则
1. **严禁**在回复中生成任何图片链接。
2. **尊重现状**：如果用户要求调整图层，通过 `[当前地图状态]` 找到对应的 ID/别名，调用工具并在回复中包含 JSON 指令。
3. 工具调用后，给出专业的中文分析说明。"""
