...
class SessionPlan(BaseModel):
    ...
    #: ADR-0208: envelope-monotonic event sequence for the canonical journal
...
def format_session_plan_projection(
    ...
    if isinstance(product, dict):
        ...
        if line:
            product_line = "\n" + line
    # ADR-0183：goal satisfaction 单行（additive；只读 finalizer 落账的
    # goal_satisfaction 块 —— 任务语义 verdict/signal/缺失码。块缺席
    # （未终验过/旧块）零漂移；投影失败只少一行，首行契约不变）。
    goal_line = ""
    try:
        ...
        if isinstance(product, dict):
            goal_line = goal_line_from_block(product.get("goal_satisfaction"))
            if goal_line:
                goal_line = "\n" + goal_line
    ...
    try:
        ...
        progress_line = ""
    # V7（ADR-0134 D1）：HarnessRuntime 任务级阶段单行（additive；状态块
    ...
    try:
        ...
        runtime_line = ""
    # V7（ADR-0134 D2）：计划版本/replan 挂起/最小重算清单单行（additive；
    ...
    if tool_name == "webgis_map_intent":
        ...
        if plan.gis_chapter and old_key and new_key and old_key != new_key:
            ...
            new = SessionPlan(
                ...
                # ADR-0208：事件 seq 是信封单调计数器，必须随台账一起迁移
