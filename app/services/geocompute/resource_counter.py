...
- **残余漂移（known drift，如实披露）**：
  ...
  结论（诚实方向性）：本层在**自身故障**（Redis 不可用/超时/退避窗口）
  时 fail-open **多放行**；但 (a) 的崩溃残留读数会让投影用量虚高 ——
  在残留存活的窗口内（**至多一个 TTL**，默认 1h）可能对无害请求
  **多拒绝**。即：fail-open 只覆盖"层自身故障"，不覆盖"上游进程崩溃
  的残留"；这正是本层只能做 advisory（拒绝必须携带 counter 证据、由
  调用方如实透出）、不能替代 L1 准入的原因之一。
"""
