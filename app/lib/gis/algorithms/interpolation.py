...
ALGORITHMS: List[AlgorithmDescriptor] = [
        ...
        AlgorithmDescriptor(
            ...
            backend_variants=[
                BackendVariant(id="numpy_batched", backend="numpy", deterministic=True,
                               min_features=8, max_features=500_000,
                               approximation_class="exact",
                               notes="与 kriging 同底预算（solve chunk 1024，max_pairs 20 万）；协方差形式无约束行"),
            ],
            cancellation_profile="chunk_boundary",
            tolerance=NumericalTolerance(rtol=1e-6, atol=1e-9, policy="conformance"),
            ),
        ...
        AlgorithmDescriptor(
            ...
            backend_variants=[
                BackendVariant(id="numpy_batched_drift", backend="numpy", deterministic=True,
                               min_features=8, max_features=500_000,
                               approximation_class="exact",
                               notes="与 kriging 同底预算 + 漂移数组（solve chunk 1024，max_pairs 20 万）"),
            ],
            cancellation_profile="chunk_boundary",
            tolerance=NumericalTolerance(rtol=1e-6, atol=1e-9, policy="conformance"),
            ),
        ...
        AlgorithmDescriptor(
            ...
            conformance_tests=[
                ...
                "tests/unit/lib/test_sgs_batched_v5.py::TestSGSBatched::test_same_seed_bitwise_reproducible",
                "tests/unit/lib/test_sgs_batched_v5.py::TestSGSBatched::test_statistical_differential_vs_reference",
                "tests/unit/lib/test_sgs_batched_v5.py::TestReferenceConditioningFix::test_multichunk_conditioning_anchor_strengthens",
            ],
            uncertainty_producer_tests={
                "monte_carlo_summary": "tests/unit/lib/test_kriging_simulation_v4.py::test_sgs_ensemble_quantiles_ordered",
            },
            resource_envelope=ResourceEnvelope(hard_max_cells=20_000_000, hard_max_features=200_000, bytes_per_cell=8, notes="R×N ensemble 单元硬顶 2000 万；reference R≤2000、N≤20 万；batched N 动态上限=min(200万, 预算/R)"),
            backend_variants=[
                BackendVariant(id="numpy_batched", backend="numpy", deterministic=True,
                               min_features=8, max_features=2_000_000,
                               approximation_class="approximate",
                               notes="窗口单位 = 目标格点（ScaleProfile.raster_cells 触发）；P 组共享路径 + chunk 堆叠 solve（O(P·T/B) 次）；动态上限 min(200万, ensemble 预算/R)；O(R·T) 输出矩阵为必然内存"),
                BackendVariant(id="numpy_sequential", backend="numpy", deterministic=True,
                               min_features=8, max_features=200_000,
                               approximation_class="approximate",
                               notes="reference 逐节点条件循环；N≤20 万（纯 Python 可操作规模）；同 seed 逐位 oracle 锚定"),
            ],
            cancellation_profile="chunk_boundary",
        ...
        AlgorithmDescriptor(
            ...
            conformance_tests=[
                ...
                "tests/unit/lib/test_cokriging_lmc_batched_v5.py::test_bitwise_equal_well_conditioned",
                "tests/unit/lib/test_cokriging_lmc_batched_v5.py::test_exactly_singular_row_isolated_to_fallback",
            ],
            ...
            backend_variants=[
                BackendVariant(id="numpy_batched", backend="numpy", deterministic=True,
                               min_features=8, max_features=500_000,
                               approximation_class="exact",
                               notes="窗口单位 = 目标数（ScaleProfile.raster_cells/features 等价——系统规模由主+次样本决定）；堆叠批量求解（隔离 = LinAlgError ∨ 非有限），与逐行解同环境逐位一致（differential oracle 钉死）"),
                BackendVariant(id="numpy_lmc_exact", backend="numpy", deterministic=True,
                               min_features=8, max_features=500_000,
                               approximation_class="exact",
                               notes="次变量 >2 万点确定性抽稀；LMC_MAX_SECONDARY=50 万硬顶；k1+k2+2 约束系统"),
            ],
            cancellation_profile="chunk_boundary",
            tolerance=NumericalTolerance(rtol=1e-9, atol=0.0, policy="conformance"),
            ),
        ...
        AlgorithmDescriptor(
            ...
            conformance_tests=[
                ...
                "tests/unit/lib/test_st_kriging_batched_v5.py::TestSTBatchedDifferential::test_bitwise_equal_no_padding",
                "tests/unit/lib/test_st_kriging_batched_v5.py::TestSTBatchedDifferential::test_window_relax_matches_v4_semantics",
                "tests/unit/lib/test_st_kriging_batched_v5.py::TestSTCrossValidation::test_temporal_forward_no_future_leakage",
            ],
            ...
            backend_variants=[
                ...
                BackendVariant(id="numpy_st_windowed", backend="numpy", deterministic=True,
                               min_features=12, max_features=300_000,
                               approximation_class="exact",
                               notes="时空三元组 + 时间窗邻域矩阵；ST_MAX_SAMPLES=30 万硬顶"),
            ],
            cancellation_profile="chunk_boundary",
            tolerance=NumericalTolerance(rtol=1e-9, atol=0.0, policy="conformance"),
            ),
