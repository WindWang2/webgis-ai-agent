...
ALGORITHMS: List[AlgorithmDescriptor] = [
        ...
        AlgorithmDescriptor(
            ...
            cpu_cost="medium", memory_cost="medium", io_cost="low",
            preferred_execution_policy="THREAD",
            algorithm_family="spatial_aggregation",
            assumptions=["H3 分辨率显式参数；计数/数值聚合（sum/mean）显式声明",
                         "点落格按 H3 索引包含关系"],
            limitations=["跨分辨率的单元面积不同（对比需归一化，披露）"],
            crs_class="CRS_AGNOSTIC",
            random_seed_policy="deterministic",
            scientific_status="VALIDATED",
            conformance_tests=[
                "tests/test_h3_binning_caching.py::test_h3_binning_second_call_cache_hit",
            ],
        ...
        AlgorithmDescriptor(
            ...
            preferred_execution_policy="THREAD",
            algorithm_family="spatial_aggregation",
            assumptions=["等矩网格（目标 CRS 米制格宽）；计数/数值聚合显式"],
            limitations=["格网在投影平面定义（高纬变形与投影一致，披露）"],
            crs_class="GEOGRAPHIC_OK",
            random_seed_policy="deterministic",
            scientific_status="VALIDATED",
            conformance_tests=[
                "tests/unit/lib/test_fishnet_vector.py::test_square_cells_are_meter_sized",
                "tests/unit/lib/test_fishnet_vector.py::test_cell_size_le_zero_rejected",
            ],
            compatible_map_models=["aggregate_grid"],
