...
def reclassify(
    ...
    if out_path == raster_path:
        out_path = raster_path + ".reclassified.tif"

    with rasterio.open(raster_path) as src:
        data = src.read(1)
        profile = src.profile.copy()
        profile.update({"driver": "GTiff", "compress": "lzw", "tiled": True, "blockxsize": 256, "blockysize": 256})
...
def raster_calculator(
    ...
    out_path = raster_a.replace(".tif", "_calc.tif")
    if out_path == raster_a:
        out_path = raster_a + ".calc.tif"
    ...
    with rasterio.open(raster_a) as src_a:
        ...
        mask = (data_a != nodata_a) & (data_b != nodata_a)
        if nodata is None:
            out_nodata = nodata_a if nodata_a is not None else 0
        else:
            out_nodata = nodata

        safe_a = np.where(mask, data_a, 0)
        safe_b = np.where(mask, data_b, 0)
        result = ne.evaluate(expression, local_dict={"A": safe_a, "B": safe_b})
...
def resample_raster(
    ...
    out_path = raster_path.replace(".tif", "_resampled.tif")
    if out_path == raster_path:
        out_path = raster_path + ".resampled.tif"
