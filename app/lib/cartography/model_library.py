...
SEED_MAP_MODELS: List[MapModel] = [
    ...
    MapModel(
        ...
        purpose_zh="以高度（米）编码数值的多边形立体表达，辅以专题设色与 3D 视角推荐",
        geometry_kinds=["polygon"],
        ...
        qgis_renderer="2.5D",
        runtime_status="native",
        accepted_artifact_types=["admin_aggregate_table", "polygon_feature_set", "admin_boundary_set"],
        recommended_components=["legend", "title"],
        supported_template_kinds=["thematic"],
        export_compatibility=["png", "pdf", "svg"],
        pitfalls_zh=[
            "高度单位是米（fill-extrusion-height），倾角大时遮挡严重需配合相机俯仰限制",
            "严禁直接将未经归一化的大数值（如千万人口）直接作为米数，应采用有界映射与极值保护",
            "矢量 SVG 导出时 perspective 3D 视图将诚实降级为 2D 专题面并标注 export_degraded=true",
        ],
        sources=[_MAPLIBRE_SPEC_URL, _QGIS_URL],
    ),
    MapModel(
        id="isoline_contour", name_zh="等值线/等值面",
        purpose_zh="连续表面/KDE/高程场的等阈值折线或多边形等值面带",
        geometry_kinds=["line", "polygon", "point", "raster"],
        maplibre_layer_type="line",
        geometry_layer_types={"line": "line", "polygon": "fill"},
        color_scheme_kind="sequential", default_palette="Inferno",
        deck_gl_layer="ContourLayer", kepler_layer="contour",
        runtime_status="native",
        accepted_artifact_types=["density_surface", "terrain_surface", "raster_surface", "point_feature_set"],
        recommended_components=["legend", "continuous_colorbar"],
        supported_template_kinds=["thematic"],
        export_compatibility=["png", "pdf", "svg"],
        pitfalls_zh=[
            "等值距建议为整齐步长以便读图；带宽过窄会产生锯齿",
            "用户显式指定的等值线等级必须 100% 精确保留，严禁自动替换",
            "等值面多边形必须建立奇偶拓扑嵌套保证洞（holes）不被覆盖",
        ],
        sources=[_DECKGL_URL, _KEPLER_URL],
    ),
]
