...
图层管理 API 路由 - 已修复版本
修复内容：
1. 移除了硬编码的假用户 (get_current_user 模拟函数)
2. 集成了 JWT 认证中间件 (app.core.auth.get_current_user)
3. 所有受保护路由现在需要有效的 JWT token，否则返回 401
"""
...
router = APIRouter()
# ==================== 图层 CRUD ====================
@router.post("/layer", response_model=LayerResponse, tags=["图层管理"])
def create_layer(
    layer: LayerCreate,
...
@router.get("/layer", response_model=LayerListResponse, tags=["图层管理"])
def list_layer(
    ...
    """
    layer_service = LayerService(db)
    layer_list, total = layer_service.list_all(
        limit=limit,
        offset=offset,
...
@router.get("/layer/{layer_id}", response_model=LayerResponse, tags=["图层管理"])
def get_layer(
    layer_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(auth_get_current_user)
...
@router.put("/layer/{layer_id}", response_model=LayerResponse, tags=["图层管理"])
def update_layer(
    layer_id: int,
...
@router.delete("/layer/{layer_id}", response_model=SuccessResponse, tags=["图层管理"])
def delete_layer(
    layer_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(auth_get_current_user)
...
# ==================== 空间分析任务 ====================
@router.post("/layer/{layer_id}/tasks", response_model=TaskResponse, tags=["空间分析"])
def create_analysis_task(
    ...
    示例参数:
    - buffer: {"distance": 100, "unit": "meters"}
...
# ==================== 元数据接口 ====================
@router.get("/layer/{layer_id}/metadata", tags=["元数据"])
