...
def attach_dataset(project_id: str, data: DatasetAttach, db: Session = Depends(get_db)):
    dataset = ProjectService.attach_dataset(db=db, project_id=project_id, attach_data=data)
    if not dataset:
        raise HTTPException(status_code=404, detail="Project not found or permission denied")
    return dataset
...
def detach_dataset(project_id: str, dataset_id: str, db: Session = Depends(get_db)):
    success = ProjectService.detach_dataset(db=db, project_id=project_id, dataset_id=dataset_id)
    if not success:
...
def list_datasets(project_id: str, db: Session = Depends(get_db)):
    return ProjectService.list_project_datasets(db=db, project_id=project_id)
...
def list_artifacts(project_id: str, db: Session = Depends(get_db)):
    return ProjectService.list_project_artifacts(db=db, project_id=project_id)


@router.post("/{project_id}/workflows", response_model=WorkflowResponse)
def save_workflow(project_id: str, data: WorkflowCreate, db: Session = Depends(get_db)):
    # Validate DAG topology before saving
    try:
        WorkflowEngine.validate_dag(data.graph_spec.steps)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

    workflow = ProjectService.save_workflow(db=db, project_id=project_id, workflow_data=data)
    if not workflow:
...
def list_workflows(project_id: str, db: Session = Depends(get_db)):
    return ProjectService.list_project_workflows(db=db, project_id=project_id)
...
def list_runs(project_id: str, workflow_id: Optional[str] = None, db: Session = Depends(get_db)):
    return ProjectService.list_workflow_runs(db=db, project_id=project_id, workflow_id=workflow_id)


@router.post("/{project_id}/runs/compare", response_model=RunComparisonResponse)
def compare_runs(project_id: str, run_a_id: str, run_b_id: str, db: Session = Depends(get_db)):
    runs = ProjectService.list_workflow_runs(db=db, project_id=project_id)
...
async def audit_spatial_quality(project_id: str, payload: Dict[str, Any], crs: str = "EPSG:4326"):
    geojson_data = payload.get("geojson")
    if not geojson_data:
        raise HTTPException(status_code=400, detail="Missing 'geojson' in payload")

    report = await SpatialQualityEngine.audit_dataset(geojson_data=geojson_data, dataset_id=project_id, crs=crs)
    return report.to_dict()


@router.post("/{project_id}/repair")
async def repair_spatial_dataset(project_id: str, payload: Dict[str, Any]):
    geojson_data = payload.get("geojson")
    operations = payload.get("operations", ["make_valid", "remove_empty"])
    if not geojson_data:
        raise HTTPException(status_code=400, detail="Missing 'geojson' in payload")

    repaired_geojson, logs = await SpatialRepairPipeline.repair_dataset(geojson_data, operations)
    return {
