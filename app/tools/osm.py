...
def register_osm_tools(registry: ToolRegistry):
    ...
    @tool(registry, name="query_osm_poi",
           description="查询 OpenStreetMap 中的兴趣点（POI），如餐厅、学校、医院等",
           param_descriptions={
               "area": "区域名称，如'北京'、'海淀区'",
               "category": "POI 类别，如 restaurant/school/hospital/park/bank",
               "limit": "返回数量上限，默认50"
           })
    ...
    @tool(registry, name="query_osm_roads",
           ...
           param_descriptions={
               ...
               "road_type": "道路类型，如 highway/residential/primary/secondary",
               "limit": "返回数量上限，默认100"
           })
    async def query_osm_roads(bbox: str, road_type: str = "highway", limit: int = 100) -> dict:
        parts = [float(x.strip()) for x in bbox.strip("[]()").split(",")]
        if len(parts) != 4:
            return {"error": "bbox 格式错误，需要 [south, west, north, east]"}

        query = f"""
        way["highway"="{road_type}"]({parts[0]},{parts[1]},{parts[2]},{parts[3]});
        out {limit};
        """
        geojson = await _query_overpass(query)
        return {
    ...
    async def query_osm_buildings(bbox: str, limit: int = 100) -> dict:
        ...
        query = f"""
        way["building"]({parts[0]},{parts[1]},{parts[2]},{parts[3]});
        out {limit};
        """
        geojson = await _query_overpass(query)
        return {
    ...
    async def query_osm_boundary(name: str, admin_level: int = 8) -> dict:
        query = f"""
        area[name="{name}"]["admin_level"="{admin_level}"]->.searchArea;
