...
def register_plan_mode_tools(registry: ToolRegistry):
    ...
    @registry.tool(
        ...
        memory_class="light",
        scale_class="small",
        tags=["计划", "规划", "多步", "plan mode", "任务分解", "编排"],
        output_semantic_type="text",
        result_size_policy="inline_small",
        failure_modes=["invalid_args"],
        summary="提交结构化多步分析计划（不执行），校验工具名与 DAG 后返回 plan_id 与摘要，等用户确认后再 execute_plan。",
        examples=["把这个缓冲→裁剪→热点分析拆成计划，先给我确认", "先列出步骤计划，别直接执行"],
        anti_examples=["一两步就能完成的小任务也提计划"],
    )
    ...
    @registry.tool(
        ...
        memory_class="light",
        scale_class="small",
        tags=["计划", "状态", "查询", "plan status", "进度"],
        output_semantic_type="text",
        result_size_policy="inline_small",
        required_context=["session_plan"],
        failure_modes=["missing_data"],
        summary="查询已提交计划的当前状态（pending/running/completed/failed 等），用于回看失败步骤或避免重复执行。",
        examples=["看看刚才那个计划跑到哪一步了", "检查这个计划是否已经执行过"],
    )
