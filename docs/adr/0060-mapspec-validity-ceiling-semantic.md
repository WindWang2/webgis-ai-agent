...
**Decision:** Delete `COMPILE_VALID` and `RUNTIME_VALID` from MapSpecValidity. `is_valid` remains `tier >= SEMANTIC_VALID`. TS `compile-report` stays a tool artifact (`webgis_compile_maplibre`); do not auto-compile on evaluate. Runtime evidence is Observed Map / cartography (ADR-0054, ADR-0061), not a validity rung. This supersedes the ladder paragraph of ADR-0051 decision 1; the rest of ADR-0051 stands.
