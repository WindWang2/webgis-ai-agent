...
| D9 | 09-13 | SVG 内容态标记 `data-export-content="vector\|mixed"`（§5 mixed 语义）；review-r1 修正：mixed 仅由**层近似**驱动（编译器新增 `layer_approximated_heatmap/extrusion` 诊断 + 产物内 data-export-degraded 标记），label 截断/预算诊断不再误标 mixed；heatmap 近似不再无声 | S2 独立 Spec 审查发现初版用诊断计数判定 → 长标签导出被误标 mixed、heatmap 近似反被标 vector。位图逐层嵌入需 live map 访问，超出纯编译器边界，保持复合回退路径（vector_svg_fallback_raster）；未知层类型静默跳过登记为已知限制（编译器层面无 owner 可发诊断，后续由 schema 校验上游拦截）。 |
