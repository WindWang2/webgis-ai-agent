...
pytest markers：`heavy`（重依赖 geopandas/numpy/rasterio）、`perf`（性能回归基线）、`cartography`（制图闭环门禁）、`real_services`（需真实 Postgres/Redis/Celery，不可达自跳过）。本地跑单类：`pytest -m heavy` 等。CI 后端覆盖率闸为 75%（`--cov-fail-under`，`pytest.ini` 默认开启 `--cov=app`）。
