# WebGIS AI Agent - 开发者本地调试手册 (v0.1.2)

本项目经历了底层的核心演变，当前架构深度结合了**一切皆 Agent (Everything is Agent)** 的哲学。系统通过 **Agent CNS (中枢神经系统)** 实现感官同步与全称异步计算。请仔细遵循以下启动流程。
...
打开 `.env` 文件补充以下必须的核心配置：
- `LLM_API_KEY`: 支持工具调用的 LLM 密钥 (DeepSeek / OpenAI 兼容)
- `REDIS_PASSWORD`: Redis 密码（本地开发必须设置，docker-compose.yml 强制要求）
- `DB_PASSWORD`: PostgreSQL 密码（本地开发必须设置）
- `JWT_SECRET_KEY`: JWT 签名密钥（留空则自动生成随机值，重启后失效）
...
为了简化开发流程，项目引入了统一的开发指令。你不再需要手动开启三个终端，只需一个指令即可拉起整个 CNS 生态位：
