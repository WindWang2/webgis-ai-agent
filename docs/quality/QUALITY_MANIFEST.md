...
- 工具行为证据：dispatch **152** / mention 141 / none 2（dispatch 覆盖率 52%）
- findings 棘轮：**FAIL**（dispatch 下限 151，当前 152；active waivers 0，过期 0，被豁免 findings 0 —— 豁免项仍在上方法量清单中可见）
  - 违规：`ALGO_HEAVY_NO_VARIANTS` 当前 7 > 基线 0
  - 违规：`TOOL_DESCRIPTOR_INCOMPLETE` 当前 3 > 基线 0
